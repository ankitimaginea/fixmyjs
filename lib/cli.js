...
function getMergedCommanderConfig(config){
  var fixmyjs_keys = ['indentPref', 'indent']
  var fixmyjs_commander_options = {}

  for( var index in fixmyjs_keys){
    var key = fixmyjs_keys[index]
    if(commander[key]){
      fixmyjs_commander_options[key] = commander[key]
    }
  }

  return fu.merge(config, fixmyjs_commander_options)
}
...
function genFixForFile(file, config) {
  return function () {
    ...
    var fmjOptions = getMergedCommanderConfig(config)
...
commander
  ...
  .option('-i, --indent <n>', 'Your indentation count')
